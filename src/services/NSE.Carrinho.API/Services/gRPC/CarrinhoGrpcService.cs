...
namespace NSE.Carrinho.API.Services.gRPC
{
    ...
    public class CarrinhoGrpcService : CarrinhoCompras.CarrinhoComprasBase
    {
        ...
        public override async Task<CarrinhoClienteResponse> ObterCarrinho(ObterCarrinhoRequest request, ServerCallContext context)
        {
            ...
            return MapearCarrinhoClienteToProtoResponse(carrinho);
        }

        private async Task<CarrinhoCliente> ObterCarrinhoCliente()
        {
            return await _context.CarrinhoCliente.Include(x => x.Itens).FirstOrDefaultAsync(x => x.ClienteId == _user.ObterUserId());
        }

        private static CarrinhoClienteResponse MapearCarrinhoClienteToProtoResponse(CarrinhoCliente carrinho)
        {
