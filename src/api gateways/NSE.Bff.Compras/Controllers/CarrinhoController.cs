...
namespace NSE.Bff.Compras.Controllers
{
    ...
    public class CarrinhoController : MainController
    {
        ...
        public async Task<int> ObterQuantidadeCarrinho()
        {
            ...
            return quantidade?.Itens.Sum(item => item.Quantidade) ?? 0;
        }
        ...
        private async Task ValidarItemCarrinho(ItemProdutoDTO produto, int quantidade)
        {
            ...
            if (quantidade > produto.QuantidadeEstoque)
                AdicionarErroProcessamento($"O produto {produto.Nome} possui {produto.QuantidadeEstoque} unidades no estoque, e você selecionou {quantidade}");
        }
    }
}
